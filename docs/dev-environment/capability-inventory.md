...
**Última actualização**: 2026-04-06 (PHP/Laravel/Composer)  
...
| `php-runtime` | PHP (CLI) | P1 | Maintainer imagem | mise `php@8.4` | 8.4.19 | [PHP 8.4 releases](https://www.php.net/releases/) | ok | Patch via rebuild mise; extensões mínimas [Laravel 12.x Server Requirements](https://laravel.com/docs/12.x/deployment#server-requirements) verificadas no build com `/usr/local/bin/verify-php-laravel-extensions.sh` |
| `node-runtime` | Node.js | P1 | Maintainer imagem | mise `node@22` | 22.22.2 | [Node 22.x releases](https://nodejs.org/) | ok | v22 LTS line |
| `nvim-editor` | Neovim | P1 | Maintainer imagem | tarball oficial GitHub (`nvim-linux-x86_64.tar.gz`, pin `ARG NEOVIM_VERSION` no `docker/Dockerfile`) | 0.12.1 | [Neovim stable](https://github.com/neovim/neovim/releases) | ok | Alinhado a [neovim.io/doc/install](https://neovim.io/doc/install/); `/opt/nvim-linux-x86_64/bin/nvim` + symlink `/usr/local/bin/nvim` (PATH sem depender de login shell) |
...
| `composer-cli` | Composer | P2 | Maintainer imagem | binário no prefixo mise `php@8.4` | 2.9.5 | [Composer](https://getcomposer.org/) | ok | `command -v composer`; PHP efectivo 8.4.x mise; pacote `unzip` no sistema para `composer diagnose` |
| `laravel-installer` | Laravel Installer (`laravel`) | P3 | Maintainer imagem | `composer global require laravel/installer` em `COMPOSER_HOME=/usr/local/share/composer` | 5.25.2 | [Packagist laravel/installer](https://packagist.org/packages/laravel/installer) | ok | Build da imagem requer rede para Packagist; `laravel new` / `composer create-project` requerem rede em runtime |

### Resumo P1 (auditoria 2026-Q2-01)

- Itens P1: **3** (`php-runtime`, `node-runtime`, `nvim-editor`).  
- Em conformidade directa: **3/3**.  
- Excepção documentada: **0/3**.  
